...
exports.proposalHandler = async (decryptedMessage, socket) => {
  ...
  let connection = await indy.connections.searchConnections({ myVerkey: recipient_verkey });
  ...
  try {
    ...
    console.log(
      'Presentation Exchange Id created in proposal handler: ',
      presentationExchangeRecord.presentationExchangeId
    );
    ...
    socket.emit('notification', {
      ...
      type: 'proposal',
      message: 'You have a new presentation proposal!',
      record: presentationExchangeRecord,
    });
  } catch (error) {
    // problem report
    await sendHandlerProblemReport(connection, {
...
exports.requestHandler = async (decryptedMessage, socket) => {
  ...
  try {
    let presentationRequest = JSON.parse(
      Buffer.from(message['request_presentations~attach'][0]['data']['base64'], 'base64').toString(
        'ascii'
      )
    );
    ...
    try {
      ...
      if (
        presentationExchangeRecord.state !=
        presentationsIndex.PresentationExchangeState.ProposalSent
      ) {
        throw {
    ...
    socket.emit('notification', {
      protocol: 'presentation',
      type: 'request',
      message: 'You have a new presentation request!',
      record: presentationExchangeRecord,
    });
  } catch (error) {
    // problem report
...
exports.presentationHandler = async (decryptedMessage, socket) => {
  ...
  try {
    ...
    if (
      presentationExchangeRecord.state != presentationsIndex.PresentationExchangeState.RequestSent
    ) {
      throw {
    ...
    let presentation = JSON.parse(
      Buffer.from(message['presentations~attach'][0]['data']['base64'], 'base64').toString('ascii')
    );
    ...
    presentationExchangeRecord.state =
      presentationsIndex.PresentationExchangeState.PresentationReceived;
    presentationExchangeRecord.updatedAt = indy.utils.getCurrentDate();
    await indy.wallet.updateWalletRecordValue(
      indy.recordTypes.RecordType.PresentationExchange,
      presentationExchangeRecord.presentationExchangeId,
    ...
    socket.emit('notification', {
      ...
      type: 'presentation',
      message:
        "You received a new presentation. Go to your presentation exchanges to check it's validity!",
      record: presentationExchangeRecord,
    });
  } catch (error) {
    // problem report
...
exports.acknowledgeHandler = async (decryptedMessage, socket) => {
  const { message, recipient_verkey, sender_verkey } = decryptedMessage;
  ...
  try {
    ...
    if (
      presentationExchangeRecord.state !=
      presentationsIndex.PresentationExchangeState.PresentationSent
    ) {
      throw {
...
exports.problemReportHandler = async (decryptedMessage, socket) => {
  const { message, recipient_verkey, sender_verkey } = decryptedMessage;
  ...
  const connection = await indy.connections.searchConnections({ myVerkey: recipient_verkey });
  ...
  switch (message.description.code) {
    ...
    case 'acknowledge_processing_error':
      ...
      presentationExchangeRecord.state =
        presentationsIndex.PresentationExchangeState.PresentationVerified;
      break;
  ...
  socket.emit('notification', {
    protocol: 'presentation',
    type: 'problemReport',
    message: 'A problem has occured with the other party in one of your presentation exchanges!',
    record: presentationExchangeRecord,
  });
