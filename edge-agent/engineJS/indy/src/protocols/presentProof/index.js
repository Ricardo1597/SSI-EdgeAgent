...
exports.proverSendProposal = async (presentationExchangeRecord) => {
  ...
  const presentationProposalMessage = JSON.parse(
    presentationExchangeRecord.presentationProposalDict
  );
...
exports.verifierCreateAndSendRequest = async (
  ...
  if (
    state != PresentationExchangeState.Init &&
    state != PresentationExchangeState.ProposalReceived
  ) {
    throw new Error(`Invalid state trasition.`);
  }
...
const getCredentials = async (attrCreds, reqAttrs, predsCreds, reqPreds) => {
  ...
  console.log('reqReferents: ', reqReferents);
  for (var referent in predsCreds) {
    ...
    if (referent in reqPreds && 'non_revoked' in reqPreds[referent])
...
const getNonRevocationInterval = async (credentials, reqReferents) => {
  ...
  for (var referentKey in reqReferents) {
    ...
    if (!credentials[credentialId]['rev_reg_id']) continue;

    const revRegId = credentials[credentialId]['rev_reg_id'];
    const referentNonRevocInterval =
      'non_revoked' in referented ? referented['non_revoked'] : nonRevocInterval;
...
const getRevocationStates = async (revocRegs, revocRegDeltas) => {
  ...
  for (const key in revocRegDeltas) {
    ...
    const blobReaderHandler = await indy.blobStorage.createTailsReader(
      revReg['value']['tailsLocation']
    );
    try {
...
exports.proverCreatePresentation = async (presentationExchangeRecord, reqCredentials) => {
  ...
  console.log('revocRegs: ', revocRegs);
    ...
    identifiers.map(async (identifier) => {
      ...
      if (!(identifier['schema_id'] in schemas)) {
        schemas[identifier['schema_id']] = (
          await indy.ledger.getSchema(null, identifier['schema_id'])
        )[1];
      }

      if (!(identifier['cred_def_id'] in credDefs)) {
        credDefs[identifier['cred_def_id']] = (
          await indy.ledger.getCredDef(null, identifier['cred_def_id'])
        )[1];
      }

      if (identifier['rev_reg_id']) {
        if (!(identifier['rev_reg_id'] in revRegDefs)) {
          revRegDefs[identifier['rev_reg_id']] = (
            await indy.ledger.getRevocRegDef(null, identifier['rev_reg_id'])
          )[1];
        }
...
exports.createPresentationExchangeRecord = (
  connectionId,
  message,
  initiator,
  role,
...
exports.removePresentationExchangeRecord = async (id) => {
  ...
  const [message, endpoint] = await indy.messages.prepareMessage(rejectMessage, connection);
  indy.messages.sendMessage(message, endpoint);

  await indy.wallet.deleteWalletRecord(indy.recordTypes.RecordType.PresentationExchange, id);
};
