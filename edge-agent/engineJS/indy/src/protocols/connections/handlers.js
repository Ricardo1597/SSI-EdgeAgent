...
exports.requestHandler = async (decryptedMessage, socket) => {
  ...
  try {
    ...
    if (!invitation.isActive) {
      // Cannot accept request for inactive invitation
      throw {
        externalMessage: 'Invitation not valid.',
    ...
    if (!invitation.isMultiuse && invitation.timesUsed > 0) {
      // Cannot use singleuse invitation more than once
      throw {
        externalMessage: 'Invitation not valid.',
    ...
    await connectionsIndex.addConnection(connection.connectionId, JSON.stringify(connection), {
    ...
    const messageToSend = await indy.wallet.pack(
...
exports.responseHandler = async (decryptedMessage, socket) => {
  ...
  try {
    ...
    if (autoAccept) {
      connection = await indy.connections.createAndSendAck(connection.connectionId);
      // Emit event to client-side
      socket.emit('notification', {
        protocol: 'connection',
        type: 'ack',
        message: 'New connection added to your active connections!',
        record: connection,
      });
    } else {
      // Emit event to client-side
      socket.emit('notification', {
        protocol: 'connection',
        type: 'response',
        message: 'One of your connection requests was accepted!',
        record: connection,
      });
    }
  } catch (error) {
...
exports.problemReportHandler = async (decryptedMessage, socket) => {
  ...
  socket.emit('notification', {
    protocol: 'connection',
    type: 'problemReport',
    message: 'A problem has occured with the other party in one of your connection exchanges!',
    record: connection,
  });
...
const validateConnectionField = (message, sender_verkey) => {
  if (!message.connection) throw 'Invalid message';

  if (!message.connection.did) throw 'No DID provided.';
  if (!message.connection.did_doc) throw 'No DIDDoc provided.';
  ...
  if (!service) throw 'No communication service provided.';
