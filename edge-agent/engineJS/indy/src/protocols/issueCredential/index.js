...
exports.holderCreateAndSendProposal = async (
  connectionId,
  comment,
  attributes,
  schemaId,
...
exports.holderCreateAndSendRequest = async (credentialExchangeRecord) => {
  ...
  const [, credDef] = await indy.ledger.getCredDef(
    null,
    credentialExchangeRecord.credentialDefinitionId
  );
  const credOffer = credentialExchangeRecord.credentialOffer;
  if (!credOffer['nonce']) throw new Error('Missing nonce in credential offer');
...
exports.issuerCreateAndSendCredential = async (
...
exports.createAndSendAck = async (credentialExchangeRecord) => {
  ...
  const credentialAckMessage = messages.createCredentialAckMessage(
    credentialExchangeRecord.threadId
  );
...
exports.revokeCredential = async (revocRegId, credRevId, publish) => {
  ...
  if (publish) {
    ...
    const [
      delta,
      invalidCredRevIds,
    ] = await indy.wallet.issuerRevokeCredentials(revocRegRecord.tailsLocalPath, revocRegId, [
      credRevId,
    ]);
...
exports.sendRevocationNotification = async (recordId, comment) => {
  ...
  const credentialRevokedMessage = messages.createCredentialRevokedMessage(
    credentialExchangeRecord.threadId,
    comment
  );
...
exports.createCredentialExchangeRecord = (
  connectionId,
  message,
  initiator,
  role,
...
exports.removeCredentialExchangeRecord = async (id) => {
  ...
  await indy.wallet.deleteWalletRecord(indy.recordTypes.RecordType.CredentialExchange, id);
};
...
const createEncodedCredentialValues = (credentialValues, schemaAttributes) => {
  ...
  credentialValues.forEach((attribute) => {
    if (!schemaAttributes.includes(attribute['name'])) {
      throw new Error(
        `Provided credential values are missing the value for the schema attribute ${attribute['name']}`
      );
    }
