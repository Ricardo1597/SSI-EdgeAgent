...
exports.proposalHandler = async (decryptedMessage, socket) => {
  ...
  try {
    ...
    let autoOffer = false; // Change to allow user choice
    if (autoOffer) {
      ...
      socket.emit('notification', {
        protocol: 'credential',
        type: 'proposal',
        message: 'You have a new credential proposal!',
...
exports.offerHandler = async (decryptedMessage, socket) => {
  ...
  try {
    const credOffer = JSON.parse(
      Buffer.from(message['offers~attach'][0]['data']['base64'], 'base64').toString('ascii')
    );
    ...
    if (autoRequest) {
      ...
      socket.emit('notification', {
        protocol: 'credential',
        type: 'offer',
        message: 'You have a new credential offer!',
        record: credentialExchangeRecord,
      });
    }
  } catch (error) {
...
exports.requestHandler = async (decryptedMessage, socket) => {
  ...
  try {
    ...
    const credRequest = JSON.parse(
      Buffer.from(message['requests~attach'][0]['data']['base64'], 'base64').toString('ascii')
    );
    ...
    socket.emit('notification', {
      protocol: 'credential',
      type: 'request',
      message: 'You have a new credential request!',
      record: credentialExchangeRecord,
    });
...
exports.credentialHandler = async (decryptedMessage, socket) => {
  ...
  try {
    ...
    const rawCredential = JSON.parse(
      Buffer.from(message['credentials~attach'][0]['data']['base64'], 'base64').toString('ascii')
    );
    ...
    socket.emit('notification', {
      ...
      type: 'credential',
      message:
        'One of credential requests was accepted. Check your credentials to see the new one!',
      record: credentialExchangeRecord,
    });
  } catch (error) {
    // problem report
...
exports.acknowledgeHandler = async (decryptedMessage, socket) => {
  ...
  try {
    ...
    if (
      credentialExchangeRecord.state != credentialsIndex.CredentialExchangeState.CredentialIssued
    ) {
      throw {
...
exports.revocationHandler = async (decryptedMessage, socket) => {
  ...
  socket.emit('notification', {
    protocol: 'credential',
    type: 'revocation',
    message: 'One of your credentials was revoked!',
    record: credentialExchangeRecord,
  });
...
exports.problemReportHandler = async (decryptedMessage, socket) => {
  ...
  socket.emit('notification', {
    protocol: 'credential',
    type: 'problemReport',
    message: 'A problem has occured with the other party in one of your credential exchanges!',
    record: credentialExchangeRecord,
  });
