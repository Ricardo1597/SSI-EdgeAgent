...
exports.createAndStoreRevocReg = async (issuerDid, revocDefType, credDefId, tailsBasePath, maxCredNum, issuanceByDefault) => {
    if(!revocDefType) {
...
exports.publishRevocRegDef = async (revocRegId) => {
    let revocRegRecord = await this.searchRevocRegRecord(
        {'revocRegId': revocRegId}
    );
    
    if(revocRegRecord.state !== RevocationRegistryState.Generated){
        throw new Error(`Revocation registry ${revocRegRecord.revocRegId} in state ${revocRegRecord.state}: cannot publish revocation registry definition`)
    }
...
exports.publishRevocRegEntry = async (revocRegId) => {
    ...
    if(revocRegRecord.revocRegEntry){
        if(
            ...
            throw new Error(`Revocation registry ${revocRegRecord.revocRegId} in state ${revocRegRecord.state}: cannot publish revocation registry entry`)
        }
...
exports.createRevocRegRecord = (tag, issuerDid, credDefId, revocRegId, revocRegDef, revocRegEntry, revocDefType, maxCredNum, issuanceType) => {
...
exports.getRevocRegRecord = async (id) => {
    try {
        return await indy.wallet.getWalletRecord(indy.recordTypes.RecordType.RevocationRegistry, id, {});
    } catch(error) {
...
exports.getAllRevocRegRecords = async () => {
    return await indy.wallet.searchWalletRecord(indy.recordTypes.RecordType.RevocationRegistry, {}, {});
}
