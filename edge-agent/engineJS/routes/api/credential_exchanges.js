...
router.post('/send-proposal', passport.authenticate('jwt', { session: false }), async function (
  req,
  res
) {
  try {
...
// Holder sends credential exchange proposal
router.post('/:id/send-proposal', passport.authenticate('jwt', { session: false }), async function (
  req,
  res
) {
  try {
    ...
    const [record, messageSent] = await indy.credentialExchange.holderSendProposal(
...
router.post('/send-offer', passport.authenticate('jwt', { session: false }), async function (
  req,
  res
) {
  try {
...
router.post('/:id/send-offer', passport.authenticate('jwt', { session: false }), async function (
  req,
  res
) {
  try {
...
router.post('/:id/send-request', passport.authenticate('jwt', { session: false }), async function (
  req,
  res
) {
  try {
...
router.post('/revoke', passport.authenticate('jwt', { session: false }), async function (req, res) {
  ...
  try {
    invalidCredRevIds = await indy.credentialExchange.revokeCredential(
      revocRegId,
      credRevId,
      publish
    );
...
// Issuer send credential exchange credential
router.post(
  '/:id/send-revocation-notification',
  passport.authenticate('jwt', { session: false }),
  async function (req, res) {
    try {
...
router.post('/:id/reject', passport.authenticate('jwt', { session: false }), async function (
  req,
  res
) {
  try {
