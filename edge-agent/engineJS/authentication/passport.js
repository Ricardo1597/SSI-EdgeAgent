  ...
  new localStrategy(
    ...
    async (username, password, done) => {
      try {
        ...
        return done(
          null,
          { walletHandle: walletHandle, username: username },
          { message: 'you are now logged in.' }
        );
      } catch (error) {
        if (error.indyCode === 206) {
          // WalletAlreadyOpenedError
          return done(
            null,
            { walletHandle: await indy.wallet.get(), username: username },
            { message: 'you are now logged in.' }
          );
        }
        if (error.indyCode === 204 || error.indyCode === 207) {
          // WalletNotFoundError || WalletAccessFailed
...
passport.use(
  ...
  new JWTStrategy(
    ...
    async (jwt_payload, done) => {
      ...
      if (
        walletHandle &&
        walletHandle == (await indy.wallet.get()) &&
