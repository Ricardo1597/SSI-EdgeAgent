...
const Dashboard = ({ enqueueSnackbar, closeSnackbar, accessToken }) => {
  ...
  useEffect(() => {
    setFilteredDids(dids.filter((did) => did.metadata.alias.toLowerCase().includes(filter)));
  }, [filter, dids]);
  ...
  return (
    <div className={`root-background`} style={{ minHeight: 'calc(100vh - 55px)', width: '100%' }}>
      <S.Title>My Decentralized Identifiers</S.Title>
      <S.TableDiv className={`p-5`}>
        <>
